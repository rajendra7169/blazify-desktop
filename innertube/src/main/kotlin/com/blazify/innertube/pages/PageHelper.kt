...
object PageHelper {
    ...
    fun extractArtists(runs: List<Run>?): List<Artist> {
        if (runs == null) {
            Log.d("extractArtists: runs is null")
            return emptyList()
        }
        
        Log.d("extractArtists: input runs count=${runs.size}")
        runs.forEachIndexed { idx, run ->
            Log.v("  run[$idx]: text='${run.text}', hasEndpoint=${run.navigationEndpoint != null}, browseId=${run.navigationEndpoint?.browseEndpoint?.browseId}")
        }
        
        val filtered = runs.filter { run ->
            run.text.trim().isNotBlank() && run.text != " • "
        }
        Log.d("extractArtists: after separator filter count=${filtered.size}")
        ...
        if (result.isEmpty()) {
            Log.w("extractArtists: EMPTY RESULT from ${runs.size} runs")
        } else {
            Log.d("extractArtists: result count=${result.size}")
            result.forEach { artist ->
                Log.v("  artist: name='${artist.name}', id=${artist.id}")
            }
        }
